...
- Data is stored locally: `./data/` (inside the project root)
...
The default `config['datasets']` list covers all 18 datasets:
- 16 PATS-A01 entries: 8 fonts × 2 splits (e.g. `PATS-A01-Akhbar-train`, `PATS-A01-Akhbar-val`)
- 2 KHATT splits: `KHATT-train`, `KHATT-validation`

Train/val splits for PATS are defined in `./data/ocr-raw-data/PATS_A01_Dataset/pats_splits.json`
(80/20, seed=42, generated by `scripts/generate_pats_splits.py`).

`DataLoader.iter_samples()` dynamically resolves any `PATS-A01-{font}-{split}` or `KHATT-{split}` key.
No code changes are needed to add a new PATS font — just add it to `config.yaml` and regenerate splits.
