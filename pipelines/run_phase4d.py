    ...
    with open(output_path, "a", encoding="utf-8") as f:
        for ds_key in export_datasets:
            ...
            if not insights_context.strip():
                logger.warning(
                    "[%s] No insights context available for dataset type %s -- "
                    "prompt will fall back to zero-shot.",
                    ds_key, dataset_type,
                )
            ...
            logger.info(
                "Exported %d samples for %s (dataset_type=%s).",
...
def process_dataset_analyze(
    ...
    logger.info(
        "[%s] CER=%.2f%%  WER=%.2f%%  |  no-diac CER=%.2f%%  WER=%.2f%%",
        dataset_key,
    ...
    if p2_metrics is not None:
        ...
        comparison = {
            ...
            **(_nd_comparison_block(p2_nd, corrected_result_nd, "phase4d")),
            "interpretation": (
