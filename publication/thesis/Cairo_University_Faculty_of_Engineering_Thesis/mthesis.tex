...
correction quality. Five prompt-engineering strategies are evaluated across nine
datasets---eight typewritten fonts from PATS-A01 and one handwritten corpus (KHATT)---using
Qwen3-4B-Instruct-2507. Strategies include zero-shot correction (Phase~2), confusion-matrix
injection (Phase~3), self-reflective prompting from training-split error statistics (Phase~4),
morphological post-processing via CAMeL Tools (Phase~5), and strategy combinations with
ablation (Phase~6). Results reveal that correction effectiveness is strongly font-dependent:
the best combination (confusion-matrix injection + self-reflective prompting) achieves 11.2\%
CER on PATS-A01 (down from 14.2\% OCR baseline) and 42.4\% on KHATT (down from 44.6\%).
The optimal correction strategy depends on the baseline error profile, and no single
augmentation approach dominates across all fonts and script types.}
