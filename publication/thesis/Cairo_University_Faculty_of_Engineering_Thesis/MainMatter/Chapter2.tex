...
The letter \textit{ain} (\textarabic{Ú}), for instance, appears as
\textarabic{Ú} (isolated), \textarabic{ÚÜ} (initial), \textarabic{ÜÚÜ} (medial),
and \textarabic{ÜÚ} (final).
...
In some positions it appears alone (\textarabic{Á}); in others it appears on a
carrier letter: alef (\textarabic{Ã} above, \textarabic{Å} below), waw (\textarabic{Ä}),
or ya without dots (\textarabic{Æ}).
...
\textbf{Taa marbuta.}
The feminine ending \textarabic{É} (taa marbuta) is pronounced /at/ in context
and /a/ in pause.
It is written with the same base shape as the letter ha (\textarabic{å}) but
with two dots above.
The visual similarity between \textarabic{É} and \textarabic{å} is a direct
source of OCR errors whenever the ink quality, resolution, or font rendering
is imperfect.

\textbf{Alef maqsura.}
The letter \textarabic{ì} (alef maqsura, literally ``abbreviated alef'') appears
at the end of certain words to write the long vowel /aa/.
Its shape is identical to ya (\textarabic{í}) but without the two dots below.
OCR engines regularly confuse \textarabic{ì} and \textarabic{í}, and the
...
\textarabic{È}~/~\textarabic{Ê}~/~\textarabic{Ë}~(1/2/3 dots above),
\textarabic{Ì}~/~\textarabic{Í}~/~\textarabic{Î}~(1 dot below / none / 1 dot above),
\textarabic{Ý}~/~\textarabic{Þ}~(1 dot above / 2 dots above),
\textarabic{ä}~/~\textarabic{í}~(1 dot above / 2 dots below), and others.
...
Common examples include lam-alef (\textarabic{áÇ}), which is always written as
a single glyph.
