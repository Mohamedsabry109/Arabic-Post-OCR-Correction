...
Section~\ref{subsec:ks_openiti} discusses how the classical OpenITI corpus
proved too domain-mismatched for this task.

Phase~8 of this study uses a domain-matched retrieval corpus: the Phase~2
training-split corrections themselves. Instead of dense embedding retrieval,
Phase~8 employs BM25 \cite{robertson2009bm25} with character $n$-gram
tokenisation, querying a pre-built BM25 index of training OCR texts to
retrieve the $k=5$ most similar (OCR, ground-truth) pairs for each validation
sample. These pairs are injected into the system prompt as per-sample
few-shot correction examples. This design sidesteps domain mismatch entirely,
since the retrieval corpus is drawn from the same OCR engine and text domain
as the evaluation data. The RAGIndexBuilder and RAGRetriever components are
implemented in \texttt{src/data/rag\_index.py}.
...
the OpenITI corpus was not used in the RAG experiments of this thesis.
Phase~8 instead uses a domain-matched retrieval corpus --- the Phase~2
training-split corrections from the same OCR engine and text domain --- and
employs BM25 character $n$-gram retrieval rather than dense embedding search
(see Section~\ref{subsec:rag} and Section~\ref{sec:phase8}).
