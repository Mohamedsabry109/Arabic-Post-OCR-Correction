...
The letter \textit{ain} (\textarabic{ع}), for instance, appears as
\textarabic{ع} (isolated), \textarabic{عـ} (initial), \textarabic{ـعـ} (medial),
and \textarabic{ـع} (final).
...
In some positions it appears alone (\textarabic{ء}); in others it appears on a
carrier letter: alef (\textarabic{أ} above, \textarabic{إ} below), waw (\textarabic{ؤ}),
or ya without dots (\textarabic{ئ}).
...
\textbf{Taa marbuta.}
The feminine ending \textarabic{ة} (taa marbuta) is pronounced /at/ in context
and /a/ in pause.
It is written with the same base shape as the letter ha (\textarabic{ه}) but
with two dots above.
The visual similarity between \textarabic{ة} and \textarabic{ه} is a direct
source of OCR errors whenever the ink quality, resolution, or font rendering
is imperfect.

\textbf{Alef maqsura.}
The letter \textarabic{ى} (alef maqsura, literally ``abbreviated alef'') appears
at the end of certain words to write the long vowel /aa/.
Its shape is identical to ya (\textarabic{ي}) but without the two dots below.
OCR engines regularly confuse \textarabic{ى} and \textarabic{ي}, and the
...
\textarabic{ب}~/~\textarabic{ت}~/~\textarabic{ث}~(1/2/3 dots above),
\textarabic{ج}~/~\textarabic{ح}~/~\textarabic{خ}~(1 dot below / none / 1 dot above),
\textarabic{ف}~/~\textarabic{ق}~(1 dot above / 2 dots above),
\textarabic{ن}~/~\textarabic{ي}~(1 dot above / 2 dots below), and others.
...
Common examples include lam-alef (\textarabic{لا}), which is always written as
a single glyph.
...
Their limitations -- sparse counts for long contexts and inability to generalise
across syntactically similar contexts -- motivated neural approaches.

Bengio et al.\ \cite{bengio2003neural} introduced the neural probabilistic language
model, which represents words as dense real-valued vectors (embeddings) and uses
a feedforward network to predict the next word given a fixed-width context window.
This model demonstrated that neural approaches could outperform n-gram models by
generalising across similar word contexts.

Recurrent Neural Networks (RNNs) extended this to variable-length contexts.
Long Short-Term Memory (LSTM) networks \cite{hochreiter1997long} addressed the
vanishing gradient problem of vanilla RNNs with gated memory cells, enabling
effective modelling of long-range dependencies.
LSTM-based language models and encoder-decoder sequence-to-sequence models
dominated NLP in the early 2010s.
...
Transformers train orders of magnitude faster than LSTMs on parallel hardware
and scale more effectively with data and model size.
...
massive corpus of text scraped from the internet, books, code, and other sources.
Pre-training instils broad language competence, world knowledge, and the ability
to reason across many domains, all from the statistical structure of the training
corpus.
...
\textbf{Instruction tuning and RLHF.}
...
set of instruction-response pairs, teaching it to interpret instructions as tasks
and produce helpful, structured outputs.
Reinforcement Learning from Human Feedback (RLHF) further aligns the model's
outputs with human preferences by training a reward model on human comparisons
and optimising the language model against it using policy gradient methods.
The result is an instruction-tuned model that follows natural language
instructions reliably across a wide range of tasks without requiring task-specific
fine-tuning.

Instruction-tuned models are the right choice for post-OCR correction because
they can follow a correction instruction directly.
Unlike pre-trained base models, which require careful prompt engineering to
elicit a specific output format, instruction-tuned models respond predictably
to system messages of the form ``correct this text and return only the corrected
version.''
...
\begin{itemize}
  ...
  \item \textbf{Source of examples.}
        Phase~4B uses QALB corpus pairs.
        ...
        distributions -- is a potential explanation for the performance outcome
...
The domain mismatch between the classical OpenITI corpus and the modern
newspaper PATS-A01 text is a central element of the Phase~5 analysis in
Chapter~5.
...
Phase~4D operationalises a lightweight variant of this idea.
...
The rules used in this study address: hamza placement (al-qat' vs.\ al-wasl),
...
primary education.
They are stored in the \texttt{data/rules/} directory as JSONL files and loaded
by \texttt{RulesLoader} at runtime.
...
Both types of knowledge tell the model what to look for; the confusion matrix
focuses its attention by describing the engine's error patterns, while the
rules frame the correction task in terms of canonical Arabic spelling.
...
QALB is used in this study as a source of few-shot demonstration pairs for
Phase~4B.
After filtering for OCR-relevance and length (Section~\ref{subsec:kb_qalb},
Chapter~3), 528 pairs are available.
...
OpenITI is used in Phase~5 as the retrieval corpus for RAG.
Its primary limitation for this application is \emph{domain mismatch}: OpenITI
texts are classical and historical, while PATS-A01 Akhbar documents are modern
newspaper text.
The semantic and lexical overlap between medieval religious scholarship and
contemporary news reporting is low, which affects retrieval quality.
A retrieval corpus drawn from modern Arabic newspapers would be a better match
for the PATS-A01 domain.
...
\begin{enumerate}
  \item \textbf{Systematic evaluation of six isolated augmentation strategies.}
        Prior work on LLM-based OCR correction has at most compared zero-shot
        with one or two augmentation conditions.
        This study isolates six conditions (confusion matrix, orthographic rules,
        few-shot examples, morphological post-processing, self-reflective
        prompting, and RAG), each compared against the same zero-shot baseline
        using the same model, dataset, and evaluation protocol.

  \item \textbf{Self-reflective prompting for OCR correction.}
        The Phase~4D strategy -- injecting pre-computed statistics about the
  ...
  \item \textbf{Quantitative study of over-correction in Arabic OCR.}
        The finding that the LLM introduces errors on 25.06\% of previously
        correct tokens, and that this rate increases as more knowledge is injected
  ...
  \item \textbf{Arabic-specific evaluation.}
        The PATS-A01 Akhbar dataset provides a controlled typewritten Arabic
        testbed with known font characteristics and a clean ground truth, enabling
        precise attribution of CER changes to specific prompt interventions.
\end{enumerate}
