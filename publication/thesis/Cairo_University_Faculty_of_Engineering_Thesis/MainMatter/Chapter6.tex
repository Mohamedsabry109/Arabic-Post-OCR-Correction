...
A more radical cross-engine extension --- Experiment~3 --- is to replace the
Qaari OCR engine entirely with a vision--language model (VLM) operating
directly on document images without a separate OCR stage. Gemma4 (the
Gemma~4B and Gemma~27B model family) can be applied in VLM mode to transcribe
Arabic document images end-to-end. Comparing Gemma4-VLM transcription quality
against Qaari OCR across the thirteen evaluation datasets would quantify the
gap between a dedicated open-source OCR engine and a general-purpose VLM on
the same documents. Post-OCR correction of Gemma4-VLM transcriptions using
the best Experiment~2 prompt configuration would then determine whether
correction remains beneficial at higher OCR quality, or whether the improvement
seen with Qaari ($-$21.1\% relative CER on PATS-A01) shrinks as the underlying
recognition engine strengthens. Evaluation on the RDI (Research and Development
International) Arabic OCR benchmark datasets would situate these findings in
the established benchmarking literature. The central question for Experiment~3
is whether the over-correction threshold shifts with OCR quality: if the Gemma4
transcription already achieves very low CER, the threshold analysis from
Section~\ref{sec:disc_overcorrection} predicts that post-OCR correction would
enter the harmful regime for most documents, making a confidence-gated or
fully withheld correction strategy the correct choice.
